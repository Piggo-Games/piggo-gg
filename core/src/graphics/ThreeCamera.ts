  ...
  init: (world) => {
    ...
    return {
      ...
      onRender: (_, delta) => {
        ...
        const thirdPos = { x: interpolated.x - offset.x, y: interpolated.y - offset.z, z: interpolated.z + height - offset.y }
        ...
        if (camera.mode === "first") {
          ...
          if (world.game.id !== "build") camera.c.rotation.set(y + recoil * 0.03, x, 0)
        } else {
          if (camera.transition < 100) {
            camera.c.position.set(
